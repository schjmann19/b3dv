...
ConsoleCommand console_parse_command(const char *input) {
    ...
    if (input) {
        str raw = str_create(input);
        if (raw.len >= sizeof(cmd.raw_input)) {
            memcpy(cmd.raw_input, cstr(&raw), sizeof(cmd.raw_input) - 1);
            cmd.raw_input[sizeof(cmd.raw_input) - 1] = '\0';
        } else {
            memcpy(cmd.raw_input, cstr(&raw), raw.len + 1);
        }
        str_destroy(&raw);
    }
    ...
    if (!input || input[0] != '/') {
        cmd.type = CMD_CHAT;
        if (input) {
            str message = str_create(input);
            str_trim(&message);
            if (message.len >= sizeof(cmd.args)) {
                memcpy(cmd.args, cstr(&message), sizeof(cmd.args) - 1);
                cmd.args[sizeof(cmd.args) - 1] = '\0';
            } else {
                memcpy(cmd.args, cstr(&message), message.len + 1);
            }
            str_destroy(&message);
        }
        return cmd;
    ...
    if (strcmp(command, "give") == 0) {
        ...
        cmd.type = CMD_LOAD;
    } else if (strcmp(command, "createworld") == 0) {
        ...
        if (input) {
            str args = str_create(input + cmd_len);
            str_trim(&args);
            if (args.len >= sizeof(cmd.args)) {
                memcpy(cmd.args, cstr(&args), sizeof(cmd.args) - 1);
                cmd.args[sizeof(cmd.args) - 1] = '\0';
            } else {
                memcpy(cmd.args, cstr(&args), args.len + 1);
            }
            str_destroy(&args);
        }
        return cmd;
    ...
    if (has_player_target && input && *input != '\0') {
        ...
        if (looks_like_uid || looks_like_name) {
            ...
            size_t target_copy_len = strlen(potential_target);
            if (target_copy_len >= sizeof(cmd.player_target)) {
                target_copy_len = sizeof(cmd.player_target) - 1;
            }
            memcpy(cmd.player_target, potential_target, target_copy_len);
            cmd.player_target[target_copy_len] = '\0';
